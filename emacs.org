...
#+BEGIN_SRC emacs-lisp
  (defun r760-is-user-buffer()
    ...
    (interactive)
    (if (and (or (not (string-match "^\*.*\*$" (buffer-name))) (string-match "^\*scratch\*.*$" (buffer-name))) (not (or (string-equal major-mode "magit-status-mode") (string-equal major-mode "dired-mode")))) t 'nil))
#+END_SRC
